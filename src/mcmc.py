...
####################################################################################
@partial(jax.jit, static_argnums=0)
def mcmc_mode(logp_fn, x_init, key, mc_steps, mc_stddev, invsqrtw):
    """
        Markov Chain Monte Carlo sampling algorithm with sqrt(w).
    ...
    OUTPUT:
        - x: resulting batch samples, with the same shape as `x_init`.
    """
    batch, num_modes, _ = x_init.shape
    
    def step(i, state):
        x, logp, key, num_accepts = state
        key, key_proposal, key_accept = jax.random.split(key, 3)

        x_proposal = x + mc_stddev * \
                    jnp.einsum('n,bnd->bnd', 
                               invsqrtw, 
                               jax.random.normal(key_proposal, (batch, num_modes, 1))
                               )
    ...
    logp_init = logp_fn(x_init)
    x, logp, key, num_accepts = jax.lax.fori_loop(0, mc_steps, step, 
                                                  (x_init, logp_init, key, 0.)
                                                  )
...
####################################################################################
# @partial(jax.jit, static_argnums=0)
# def mcmcw(logp_fn, x_init, key, mc_steps, mc_stddev, invsqrtw):
#     """
#         Markov Chain Monte Carlo sampling algorithm with sqrt(w).
...
#         #x_proposal = x + mc_stddev * jax.random.normal(key_proposal, x.shape)
#         x_proposal = x + mc_stddev * \
#                     jnp.einsum('n,bnd->bnd', invsqrtw, jax.random.normal(key_proposal, x.shape))
