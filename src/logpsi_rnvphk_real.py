...
from .orbitals import logphi_base

####################################################################################################
def make_logpsi(flow, sp_orbitals):
    """
        Computes the logrithm of the wavefunction:
            log_psi = log_phi + 0.5*log_jacdet.
    """
    def logpsi(x, params, state_indices, w_indices):
        ## calculate logphi & logjacdet
        z, logjacdet = flow.apply(params, None, x)
        log_phi = logphi_base(sp_orbitals, state_indices, w_indices, z)
...
####################################################################################################
def make_logphi_logjacdet(flow, sp_orbitals):
    """              
    ...
    """
    def logphi(x, params, state_indices, w_indices):  
...
def make_logpsi_grad_laplacian(logpsi, 
    ...
    def logpsi_vmapped(x, params, state_indices, w_indices):
        logpsix = logpsi(x, params, state_indices, w_indices)
        #return logpsix[0] + 1j * logpsix[1]
        return logpsix

    ##==== grad_logpsi and laplacian_logpsi ====
    @partial(jax.vmap, in_axes=(0, None, 0, None, None), out_axes=0)
    def logpsi_grad_laplacian(x, params, state_indices, w_indices, key):
        """
        ...
        grad = jax.jacrev(logpsi)(x, params, state_indices, w_indices)
        # grad = grad[0] + 1j * grad[1]
        print("Computed gradient...")

        n, dim = x.shape
        x_flatten = x.reshape(-1)
        grad_logpsi = jax.jacrev(lambda x: logpsi(x.reshape(n, dim), params, state_indices, w_indices))
    ...
    def logpsi_grad_laplacian_hutchinson(x, params, state_indices, w_indices, key):

        v = jax.random.normal(key, x.shape)

        @partial(jax.vmap, in_axes=(0, None, 0, None, 0), out_axes=0)
        def logpsi_grad_random_laplacian(x, params, state_indices, w_indices, v):
            """
            ...
            grad, hvp = jax.jvp( jax.jacrev(lambda x: logpsi(x, params, state_indices, w_indices)),
                                 (x,), (v,) )
        ...
        @partial(jax.vmap, in_axes=(0, None, 0, None, 0), out_axes=0)
        def logpsi_grad_random_logjacdet(x, params, state_indices, w_indices, v):
            grad_logphi = jax.jacrev(logphi)(x, params, state_indices, w_indices)
            ...
            n, dim = x.shape
            x_flatten = x.reshape(-1)
            grad_logphi = jax.jacrev(lambda x: logphi(x.reshape(n, dim), params, state_indices, w_indices))
        ...
        return logpsi_grad_laplacian(x, params, state_indices, w_indices, v)
...
def make_logp(logpsi):
    ...
    @partial(jax.vmap, in_axes=(0, None, 0, None), out_axes=0)
    def logp(x, params, state_indices, w_indices):
        """ logp = logpsi + logpsi* = 2 Re logpsi """
        # return 2 * logpsi(x, params, state_indices, w_indices)[0]
        return 2 * logpsi(x, params, state_indices, w_indices)
...
def make_quantum_score(logpsi):
    ...
    @partial(jax.vmap, in_axes=(0, None, 0, None), out_axes=0)
    def quantum_score_fn(x, params, state_indices, w_indices):
        grad_params = jax.jacrev(logpsi, argnums=1)(x, params, state_indices, w_indices)
...
def make_quantum_force(logpsi):
    ...
    @partial(jax.vmap, in_axes=(0, None, 0, None), out_axes=0)
    def quantum_force(x, params, state_indices, w_indices):
        # logpsi2 = lambda x, params, state_indices: 2 * logpsi(x, params, state_indices, w_indices)[0]
        logpsi2 = lambda x, params, state_indices: 2 * logpsi(x, params, state_indices, w_indices)
