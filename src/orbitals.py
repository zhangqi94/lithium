...
def get_orbitals_1d(m=1.0):
    """
    Get orbitals for 1D harmonic oscillator.
    ...
    @jax.vmap
    def sp_orbitals(indices, x, w):
        return ((m*w)**(1/4)) * jnp.exp(- 0.5 * m*w * (x**2) ) * hermite(indices, jnp.sqrt(m*w) * x)

    @jax.vmap
    def sp_energies(indices, w):
        return w * (indices + 0.5)

    return sp_orbitals, sp_energies

####################################################################################################
def logphi_base(sp_orbitals, state_indices, w_indices, x):
    x = x.flatten()
    phi_values = sp_orbitals(state_indices, x, w_indices)
    logphi_values = jnp.sum(jnp.log(jnp.abs(jnp.array(phi_values))))
    return logphi_values
