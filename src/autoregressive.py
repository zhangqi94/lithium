...
class Transformer(hk.Module):
    ...
    def __call__(self, x: jnp.ndarray) -> jnp.ndarray:
        ...
        def block(x, i):
            ...
            x = x + x_attn
            x_dense = DenseBlock(self.hidden_size, 
                                 init_scale, 
                                 name=f"layer{i}_mlp")(x)
            x = x + x_dense
            return x
        
        if self.remat:
            block = hk.remat(block, static_argnums=1)
...
####################################################################################
def make_autoregressive_model(num_levels, indices_group, van_layers, van_size, van_heads, van_hidden):
    ...
    def forward_fn(state_idx):
        model = Transformer(num_levels**indices_group, nlayers, modelsize, nheads, nhidden, False)
