...
####################################################################################################
def main_speflw(args):

    print("Measuring the phonon energy spectrum under the flow model.")
...
####################################################################################################
    ...
    #========== open file ==========
    file_name = load_ckpt[:-4] + f"spec_lev{state_index_level}.txt"
...
####################################################################################################
    ...
    #========== base mode index ==========
    from src.vmc_speflw import sample_x_mcmc, get_index_basefre, make_loss, \
                                update_observable, calculate_means_and_stds

    index_basefre = get_index_basefre(sequence_length, state_index_level)
    print("index_basefre.shape:", index_basefre.shape, flush=True)
    ...
    for ii in range(index_basefre.shape[0]):
        tf1 = time.time()
        ...
        print("[", " ".join(str(x) for x in index_basefre[ii]), "]", flush=True)
